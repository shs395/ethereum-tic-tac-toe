...
const App = () => {
  ...
  const InitializeInfo = async () => {
      ...
      tempGameList.forEach((game) => {
        temp.push([
          <p
            ...
            onClick={() => {
              selectGame(game.gameAddress);
            }}
          >
            {game.gameAddress.substr(0, 5)}...{game.gameAddress.substr(-5)}
          </p>,
          <p>
            {game.status.player1.substr(0, 5)}...
            {game.status.player1.substr(-4)}
          </p>,
          <p>
            {game.status.player2.substr(0, 5)}...
            {game.status.player2.substr(-4)}
          </p>,
          <p>
            {(game.status.betAmount / 10 ** 18) * 2} ETH /{" "}
            {game.status.betAmount / 10 ** 18} ETH
          </p>,
          game.status.gameOver == true ? (
            <p>
              <Tag color="red" text="Game End" />
            </p>
          ) : game.status.player2 == 0 ? (
            <p>
              <Tag color="green" text="waiting for game" />
            </p>
          ) : (
            <p>
          ...
          game.status.winner == 0 ? (
            <p></p>
          ) : (
            <p>
              {game.status.winner.substr(0, 5)}...
              {game.status.winner.substr(-4)}
            </p>
          ),
        ]);
        console.log(temp);
    ...
    const gameAddressListDeployedNetwork =
      GameAddressListContract.networks[networkId];
    ...
    for (let i = 1; i <= 7; i++) {
      ...
      let myQuantity = await gameItemContract.methods
  ...
  const getGameAddressList = async (web3, networkId) => {
    ...
    for (let i = 0; i < gameCount; i++) {
      let item = await gameAddressListContract.methods
        .gameAddressList(i)
        .call();
      array.push(item);
    }
  ...
  const updateGameInfo = async (web3, address, tempGameList) => {
    ...
    tempGameList.forEach((game) => {
      temp.push([
        ...
        <p>
          {game.status.player1.substr(0, 5)}...{game.status.player1.substr(-4)}
        </p>,
        <p>
          {game.status.player2.substr(0, 5)}...{game.status.player2.substr(-4)}
        </p>,
        <p>
          {(game.status.betAmount / 10 ** 18) * 2} ETH /{" "}
          {game.status.betAmount / 10 ** 18} ETH
        ...
        game.status.winner == 0 ? (
          ...
          <p>
            {game.status.winner.substr(0, 5)}...{game.status.winner.substr(-4)}
          </p>
        ),
      ]);
      console.log(temp);
  ...
  const makeGame = async () => {
    ...
    ticTacToeContract
      ...
      .then(function (newContractInstance) {
        ...
        gameAddressListContract.methods
          .addGame(newContractInstance.options.address)
  ...
  const joinGame = async (tempBetAmount, currentGameAddress) => {
    ...
    await ticTacToeContract.methods.join().send({
      from: accounts[0],
  ...
  const mint = async () => {
    ...
    gameItemContract.methods
      .mint("1", "1")
      .send({ from: accounts[0], value: web3.utils.toWei("0.005") });
  };
  ...
  const gameItemList = gameItemJson.map((item) => (
    ...
    >
      ...
      >
        ...
        <p>
          {item.supplies - item.minted} / {item.supplies} left
        </p>
        <p>price: 0.05 ETH</p>
        <Button
          id="test-button-primary"
          onClick={() => {
            buyGameItem(item.tokenId);
          }}
          isFullWidth
          text="Buy now"
          theme="primary"
  ...
  return (
    <div className="App">
      <div>
        <Typography id="typo" variant="h1">
          Ethereum-tic-tac-toe
        </Typography>
        <Typography id="sub-typo" variant="h5">
          Only available on Sepolia network
        </Typography>
        ...
        >
            ...
            >
              <Information
                information={
                  accounts == null
                    ? ""
                    : accounts[0].substr(0, 5) + "..." + accounts[0].substr(-4)
                }
                topic="Your Address"
              />
            </div>
          ...
          >
            ...
            >
              <Information
                information={
                  ethBalance == null ? "" : `${ethBalance / 10 ** 18} ETH`
                }
      ...
      <TabList defaultActiveKey={1} tabStyle="bulbUnion" className="TabList">
        <Tab tabKey={1} tabName="Game" className="Tab">
          <div>
            ...
            <Typography variant="h4">
              Current Game : {currentGameAddress}
            </Typography>
                ...
                >
                  <div>
                    {mainItem == null ? (
                      ...
                      <table
                        style={{ color: mainItem.json.name.toLowerCase() }}
                      >
                        <tr id="tr1">
                          <td
                            onClick={() => {
                    ...
                    <div id="game-button-outer">
                      ...
                      {currentGameStatus.gameOver == true &&
                      currentGameStatus.winner == accounts[0] ? (
                        <Button
                          id="test-button-primary"
                          onClick={() => {
              ...
              >
                ...
                >
                  <div>
                    ...
                    <p>
                      Prize / BetAmount :{" "}
                      {(currentGameStatus.betAmount / 10 ** 18) * 2} ETH /{" "}
                      {currentGameStatus.betAmount / 10 ** 18} ETH
                    </p>
                    {mainItem == null ? (
                      <p>YourItem : None</p>
                    ) : (
                      <p>
                        Your Item : {mainItem.json.name} -{" "}
                        {mainItem.json.description}
                      </p>
                    )}
                  </div>
                </div>
        ...
        <Tab tabKey={2} tabName="Shop" className="Tab">
        ...
        <Tab tabKey={3} tabName="My Page" className="Tab">
